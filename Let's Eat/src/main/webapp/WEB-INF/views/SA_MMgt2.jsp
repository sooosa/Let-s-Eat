...
<body>
	...
	<form action="sa_selCat2">
			<th>카테고리 검색</th>
			<td><select name="selectVal">
					<optgroup label="카테고리">
						<option value="cCode" ${selectVal == 'cCode' ? 'selected="selected"' : '' }>점주회원코드</option>
						<option value="cId" ${selectVal == 'cId' ? 'selected="selected"' : '' }>아이디</option>
						<option value="cName" ${selectVal == 'cName' ? 'selected="selected"' : '' }>이름</option>
						<option value="cEmail" ${selectVal == 'cEmail' ? 'selected="selected"' : '' }>이메일</option>
		...
		<table width="100%">
			<thead>
				<tr>
					<th class="SA_FAQViewTh">점주회원코드</th>
			...
			<tbody>
				<c:forEach var="cml" items="${cmemberList}">
					<tr>
						<td class="SA_FAQViewTdLong">${cml.cCode}</td>
						<td class="SA_FAQViewTdLong"><a href="sa_cView?cCode=${cml.cCode}">${cml.cId}</a></td>
						<td class="SA_FAQViewTdLong">${cml.cName}</td>
						<td class="SA_FAQViewTdLong">${cml.cEmail}</td>
						<td class="SA_FAQViewTdLong"><c:if test="${cml.cKey==0}">
							미승인
							</c:if> <c:if test="${cml.cKey==1}">
							승인
							</c:if> <c:if test="${cml.cKey==2}">
							탈퇴
							</c:if>
							
							<c:if test="${cml.cKey==5}">
							관리자
							</c:if>
							
							</td>
						
						<c:if test="${cml.cKey==1}">	
						<td class="SA_FAQViewTdLong">
							<button onclick="location.href='sa_cDel?cCode=${cml.cCode}'">탈퇴</button>
						</td>
						</c:if>
						
						<c:if test="${cml.cKey==2 || cml.cKey==5}">	
						<td class="SA_FAQViewTdLong">
	...
	<c:if test="${paging.page>1 && not empty keyword}">
		<a href="sa_selCat2?page=${paging.page-1}&selectVal=${selectVal}&keyword=${keyword}">[이전] </a>
	</c:if>
		<c:if test="${paging.page>1 && empty keyword}">
		<a href="sa_mMgt2?page=${paging.page-1}">[이전] </a>
	</c:if>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:if test="${not empty keyword}">
					<a href="sa_selCat2?page=${i}&selectVal=${selectVal}&keyword=${keyword}">${i}</a>
				</c:if>
				<c:if test="${empty keyword}">
					<a href="sa_mMgt2?page=${i}">${i}</a>
				</c:if>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<c:if test="${paging.page<paging.maxPage}">
		<c:if test="${not empty keyword}">
		<a href="sa_selCat2?page=${paging.page+1}&selectVal=${selectVal}&keyword=${keyword}">[다음] </a>
		</c:if>
		<c:if test="${empty keyword}">
		<a href="sa_mMgt2?page=${paging.page+1}"> [다음]</a>
		</c:if>
	</c:if>
	</div>
	<br>
