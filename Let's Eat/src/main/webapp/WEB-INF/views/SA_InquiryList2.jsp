		...
		<table>
			...
			<tbody>
				<c:forEach var="i" items="${inquiryList}">
					<tr>
						<td class="SA_FAQViewTdLong" style= "width:10%">${i.cQnACode}</td>
						<td class="SA_FAQViewTdLong" style= "width:13%">${i.cQnACat}</td>
						<td class="SA_FAQViewTdLong" style= "width:44%">${i.cQnATitle}</td>
						<td class="SA_FAQViewTdLong" style= "width:13%">${fn:substring(i.cQnADate,0,10)}</td>
						<td class="SA_FAQViewTdLong" style= "width:10%">
						<c:if test="${i.cQnAKey eq '0'}">
						대기</c:if>
						<c:if test="${i.cQnAKey eq '1'}">
						완료</c:if>
						</td>
						<td class="SA_FAQViewTdLong" style= "width:10%"><button onclick="location.href='sa_inquiryView2?cQnACode=${i.cQnACode}'">보기</button></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
<!-- 페이징 처리 -->
	...
	<c:if test="${paging.page>1 && not empty keyword}">
		<a href="sa_selCat6?page=${paging.page-1}&selectVal=${selectVal}&keyword=${keyword}">[이전] </a>
	</c:if>
		<c:if test="${paging.page>1 && empty keyword}">
		<a href="sa_inquiryList2?page=${paging.page-1}">[이전] </a>
	</c:if>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:if test="${not empty keyword}">
					<a href="sa_selCat6?page=${i}&selectVal=${selectVal}&keyword=${keyword}">${i}</a>
				</c:if>
				<c:if test="${empty keyword}">
					<a href="sa_inquiryList2?page=${i}">${i}</a>
				</c:if>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<c:if test="${paging.page<paging.maxPage}">
		<c:if test="${not empty keyword}">
		<a href="sa_selCat6?page=${paging.page+1}&selectVal=${selectVal}&keyword=${keyword}">[다음] </a>
		</c:if>
		<c:if test="${empty keyword}">
		<a href="sa_inquiryList2?page=${paging.page+1}"> [다음]</a>
		</c:if>
	</c:if>
	</div>
</body>
