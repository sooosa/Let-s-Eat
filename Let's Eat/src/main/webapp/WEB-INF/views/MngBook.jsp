...
날짜선택하기 <input type="date" name="bookDate" id="date1">${mngbook.booksCode}  ${storeCode}규뀨 ${storeCode}
...
<%-- <c:if test="${empty bookDate}"><!--날짜선택 안할시 현재날짜 --> --%>
	...
	<form action="addmngbook" method="GET">
		...
		<input type="hidden" value="${storeCode}" name="booksCode">
	불러온 mngbook이 비어있으면 여기로(bookDate:  ${bookDate} ) <br>
	(insert 나 수정할때 비어있는 칸 있으면 오류남. 전부 선택해주도록 하자)
		<table>
			<tr>
				<th>시간</th>
				<th>현재 예약 수</th>
	...
	불러온 mngbook에 뭔가 있으면 update changemngbook 여기로 ${bookDate} ${mngbook.booksCode}
		<table> 
			<tr>
				<th>시간</th>
				<th>현재 예약 수</th>
