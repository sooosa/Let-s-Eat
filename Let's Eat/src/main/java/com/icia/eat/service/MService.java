...
public class MService {
		...
		if (result != null) {
			if (pwEnc.matches(mPw, encPw)) {
				if (result.getmKey() == 1) {
					...
					session.setAttribute("mloginPhone", result.getmPhone());
	...
	public String sendEmail(String mId, String mEmail) {
		...
		String str = "<h2>안녕하세요.  Let's Eat 입니다.</h2><br>" + "<h3>" + mId + "님</h3>"
	...
	// 예약 목록 메소드
	public ModelAndView reservationlist(ReservationDTO reservation, Review_SaveDTO review) {

		// 현제 예약 중인 리스트 보기
		List<ReservationDTO> bookinglist = dao.reservationlist(reservation);
		System.out.println("bookinglist : " + bookinglist);
		// 과거 예약 보기
		List<ReservationDTO> bookedlist = dao.bookedlist(reservation);
		System.out.println("reservation : " + reservation);
		System.out.println("bCode : " + reservation.getbCode());
	...
	public ModelAndView mybookDel(String bCode, String bmCode) {
		...
		if(result>0) {
			mav.setViewName("redirect:reservationlist?bmCode=" + bmCode);
		} else {
			mav.setViewName("index");
		}
	...
	public ModelAndView reviewWrite(Review_SaveDTO review) {
		...
		if(result>0) {
			mav.setViewName("redirect:myRList?rmId=" + review.getRmId());
		} else {
			mav.setViewName("index");
		}
	...
	// 내가 쓴 리뷰 목록 (마이 리뷰)
	public ModelAndView myReviewList(String rmId) {
		
		List<Review_SaveDTO> myRList = dao.myRList(rmId);
	...
	public ModelAndView myReviewDel(String rCode, String rmId) {
		...
		if(result >0) {
			mav.setViewName("redirect:myRList?rmId=" + rmId);
		} else {
			mav.setViewName("index");
		}
	...
	// 나의 문의 현황 (회원)
	public ModelAndView M_myQnAList(String mQnAmId) {
		
		List<QnADTO> mQnAList = dao.M_myQnAList(mQnAmId);
		
		mav.addObject("mQnAList", mQnAList);
		mav.setViewName("mQnAList");
